...
p_load(devtools, here, readxl, janitor, tidyverse, openxlsx, robumeta, metafor, lubridate, gt, webshot2, prismadiagramR) 
...
cm_long <- td_cm %>% 
  select(-`Current Review`) %>% 
  pivot_longer(cols = `Ahlen 2015`:`Zhang 2022`,
...
Save table 1 to `FOLDER NAME` project folder: (**specify final paths**)
...
gtsave(table1_html, filename = "table1_html.html")
...
Save table 2 to `FOLDER NAME` project folder: 
 ...
 * The only value not in our dataset is the number of records identified in the search before duplicate removal. Our last search on **DATE** yielded *5,221* records that were imported into Zotero before duplicate removal. 
...
ourformat <- format %>% 
  ...
  mutate(prismaTxt = case_when(prismaLvl == 2 & nodeType == "Node" ~ "Records identified from search:\n(n = 5,221)", #from zotero
                               ...
                               prismaLvl == 6 & nodeType == "End" ~ paste0("Reviews included in overview: \n(k = ", 
                                                                           sum(td_screen_study$eligibility_consensus == "Eligible"), "; j = ", 
                                                                           nrow(ps_elig_td), ")"),
                               prismaLvl == 2 & nodeType == "Filter" ~ paste0("Records removed before screening: \n(n = ", 
                                                                              5221-nrow(td_reference), ")"),
...
cca_heatmap(cca_inc, decimal_digits = 0, fontsize = 4.5, fontsize_diag = 3) +
  ggplot2::theme(
...
cca_heatmap(cca_elig, decimal_digits = 0, fontsize = 4.5, fontsize_diag = 3.2) +
  ggplot2::theme(
      plot.caption = ggplot2::element_text(size = 16, margin=ggplot2::margin(30,0,0,0)),
      legend.title = ggplot2::element_text(size = 16, face = "bold", vjust=4),
      legend.text = ggplot2::element_text(size = 16),
...
Create dataframe with info for Appendix 1. List of excluded reviews at the full-text eligibility stage (excluded review & reason)
...
fm_ref <- td_reference %>% 
  mutate(author = str_extract(citation, "^\\S+"),
         ...
         author_full = str_extract(citation, "^([^,(]*)"), #still needs manual cleaning of some references that are formatted differently
...
#export to format for report
#openxlsx::write.xlsx(a1, "appendix1_info.xlsx")
...
#save as html
gtsave(a1_formatted, filename = "appendix1_html.html")
...
Create dataframe with info for Appendix 2. List of excluded primary studies (excluded study & reason)
...
a2 <- ps_elig_td %>% 
  ...
  select(study, reason, explanation, decision) %>% 
...
#export to format for report
#openxlsx::write.xlsx(a2, "appendix2_info.xlsx")
...
#save as html
gtsave(a2_formatted, filename = "appendix2_html.html")
...
Create dataframe with info for Appendix 3. List of unclear/awaiting classification reviews and primary studies
...
#collapse reasons 
a3 <- a3_info %>% 
  mutate(reason = case_when(str_detect(explanation, "Ask authors") ~ "Awaiting response to author query",
                            str_detect(explanation, "No document") ~ "Unpublished document",
                            str_detect(explanation, "Not in English") ~ "No document in English",
                            TRUE ~ explanation)) %>% 
  select(study, reason, citation)

#export to format for report
#openxlsx::write.xlsx(a3, "appendix3_info.xlsx")
...
### Appendix 4. Descriptive characteristics for each included systematic review (appendix 4) 

 * don't have data on other searches, depression definition, conclusions, or references per review data

```{r per-review-descriptives}
...
a4_info <- td_review %>% 
  ...
  select(review_author_year, review_eligibility_criteria, databases_searched, 
         search_date, n_included, n_eligible, prisma_flow_diagram, review_registration, review_availability_statement) %>% 
  arrange(str_to_lower(review_author_year)) 


#MISSING TITLE (needs formatted from reference), and ALL REFERENCES (waiting to see if papaja can help with formatting), 

#merge with lists of included and eligible studies
#a4 <- left_join(a4_info, t1_stud_cols) #taking out list of studies

```

```{r a4-maptable-TEST, results ='asis'}
#attempt to replicate multiple tables
##split data frame into list of data frames
a4_dflist <- map(1:nrow(a4_info), ~a4_info[.x, ])

##transpose each data frame in the list
# a4_dflist_t <- map(a4_dflist, ~t(.))

##create gt table for each data frame in list
# a4_gtlist <- map(a4_dflist, ~ gt(.) %>% 
#                    tab_header(title = " "))
# 
# ##extract raw HTML for each gt table and concatenate into one string
# html_code <- a4_gtlist %>% 
#   map(as_raw_html) %>% 
#   reduce(paste)
# 
# ##write HTML to a file
# writeLines(html_code, "test_a4.html")
# 
# print(html_code)

#TRY AGAIN - transfer to long format
...
df_list_long <- map(a4_dflist, ~ .x %>% 
                      ...
                      mutate(variable = case_when(variable == "review_author_year" ~ "Review",
                                                  variable == "review_eligibility_criteria" ~ "Eligibility Criteria",
...
##create gt table for each data frame in list
a4long_gtlist <- map(df_list_long, ~ gt(.) %>% 
                   tab_header(title = " "))

##extract raw HTML for each gt table and concatenate into one string
html_code2 <- a4long_gtlist %>% 
  map(as_raw_html) %>% 
  reduce(paste)
##write HTML to a file
writeLines(html_code2, "test_a4long.html")
##print in document
htmltools::HTML(html_code2)



#TRY AGAIN (word table with page breaks)
#combine into one single dataframe
combined_a4 <- bind_rows(df_list_long, .id = "source") %>% 
  mutate(source = as.numeric(source)) #%>% 
  #group_by(source)

#create full gt table
gt_fulltable <- combined_a4 %>% 
  gt(rowname_col = "variable", groupname_col = "source") 

gt_preview(gt_fulltable)
#gtsave(gt_fulltable, file = "a4_long.docx")
##^^ Still no page breaks, but can do in word. 

#options(gt.row_group.sep = "\n")
# testbreaks <- combined_a4 %>% 
#   gt(id = "one",
#     groupname_col = c("source"), 
...
#     "
#   )
# gt_preview(testbreaks)
# gtsave(testbreaks, file = "a4_breaks.docx")
  
#TRYING
# gt_table_test <- map(a4_dflist, function(x) {
#   gt(t(as.data.frame(x)))
# })

# tgt_test <- function(df) {
#   df %>% 
#     gt() %>% 
#     tab_transpose()
# }
# 
# ####generate list of transposed gt tables
# gt_list_test <- map(a4_dflist, tgt_test)

#test_gtlist <- map(a4_dflist, ~ gt(t(.), rownames_to_stub = FALSE))
...
# doc <- read_docx()
# 
# walk(a4long_gtlist, function(x) {
#   add_flextable(doc, x)
#   add_page_break(doc)
# })
# gtsave(doc, "all_a4tables.docx")


#TRY AGAIN
# Create a tab_booktabs object from the gt_list
#long_table <- a4long_gtlist %>%
 # tab_booktabs()

# long_table <- bind_rows(a4long_gtlist)
# wide_table <- bind_cols(a4long_gtlist)

# # Create a new Word document
# doc <- read_docx()
# 
# # Insert the long_table into the document
# doc <- body_add_flextable(doc, long_table)
# 
# # Save the Word document
# print(doc, target = "long_table.docx")
# 
# # Save the Excel file
# write_xlsx(long_table, "long_table.xlsx")
# 
# 
# #TRY AGAIN
# # Create a temporary HTML file
# html_file <- tempfile(fileext = ".html")
# cat(html_code2, file = html_file)
# 
# # Capture a screenshot of the HTML table
...
# doc <- read_docx()
# 
# # Insert the image into the document as a paragraph
# doc <- body_add_par(doc, "", style = "Normal")
# doc <- body_add_image(doc, img)
# 
# # Save the document
...
# 
# #ANOTHER TIME
# #attempt to save as word
# library(officer)
# doc <- read_docx()
# doc <- body_add_par(doc, html_code2)
# print(doc, target = "a4.docx")
# 
# 
# 
# # Add each table to the document
# # for (i in seq_along(a4long_gtlist)) {
# #   doc <- body_add(doc, paste0("<h2>", names(a4long_gtlist)[i], "</h2>"))
# #   doc <- body_add(doc, as_pretty_html(a4long_gtlist[[i]]))
# #   doc <- body_add(doc, "<br>")
# # }
# 
# # Save the document
...
# 
# 
# # Convert the HTML code to a data frame
# df <- htmlTable::htmlTableToDataFrame(html_code2)
# 
# # Create a new Word document
# doc <- read_docx()
# 
# # Insert the data frame into the document as a table
# doc <- body_add_table(doc, df)
# 
# # Save the document
# print(doc, target = "table.docx")
...
#TRYING BREAKS for html table = FINAL
...
list_gt <- lapply(split(combined_a4, combined_a4$source), function(x) {
  gt(x) %>% 
    ...
    #tab_header(title = paste("Review", unique(x$source)))
})
...
html_code3 <- list_gt %>% 
  ...
  reduce(paste)
#print in document
htmltools::HTML(html_code3)
writeLines(html_code3, "test_breaks.html")

# #trying breaks for word table
# gt_break_table <- combined_a4 %>% 
#   gt(rowname_col = "variable", groupname_col = "source") %>% 
#   
#   tab_options(split_table = ~ .x %>% tab_group(groupname_col = "source"))
...
#TRY split
split_a4 <- split(combined_a4, combined_a4$source)

tables <- lapply(split_a4, function(x) {
  gt(x) %>% 
    tab_header(title = paste("Review", unique(x$source)))
})

tables
#could print all individually in rmd

#try stack
# tablesstack <- tab_row_group(tables[[1]], tables[[2]])
# 
# combined_table <- tables[[1]]
# for (i in 2:length(tables)) {
#   combined_table <- tab_row_group(combined_table, tables[[i]])
# }
