...
Save table 3 to `report/tables`` project folder: 

```{r t3-save}
#save as html
gtsave(t3_formatted, filename = "table_3.html", path = here("report", "tables"))
...
```{r t3-save}
#save as html
gtsave(t4_formatted, filename = "table_4.html", path = here("report", "tables"))
...
png(here("report", "figures", "figure2_heatmap.png"), width = 1000, height = 1000)
...
```{r save-f3}
#set image dimensions and file path to save
png(here("report", "figures", "figure3_heatmap.png"), width = 1000, height = 1000)
...
pdf(here("report", "figures", "figure4_forestplot.pdf"), width = 20, height = 15)
...
#   arrange(str_to_lower(author_year))
# 
# ps_author_year <- bibdf_format$author_year
# ps_title <- bibdf_format$title
# 
# # Create a table of the references in APA format
# apa_refs <- sapply(1:nrow(bibdf_format), function(i) {
#   # Get the ith reference
#   ref <- bibdf_format[i, ]
#   # Format the reference in APA style
#   ref_string <- paste0(
#     ref[["author_unlist"]], " (", ref[["year"]], "). ", 
...
# 
# #as.data,frame
# aparef_df <- as.data.frame(apa_refs) %>% #MOSTLY IN RIGHT FORMAT - TRY ADDING ITALICS AND INITIALIZE NAMES
#   mutate(apa_refs = str_replace_all(apa_refs, "\\(NA\\)", ""), 
#          apa_refs = str_replace_all(apa_refs, "NA", ""),
#          apa_refs = str_replace_all(apa_refs, "--", "-"),
#          apa_refs = str_replace_all(apa_refs, ", ,", "")) %>% 
#   arrange(str_to_lower(apa_refs)) %>% 
#   cbind(ps_author_year) %>% 
#   cbind(ps_title) %>% 
#   rename(study_author_year = ps_author_year)
#   
...
Save appendix 5 to `report/appendices`` project folder: 

```{r a5-save}
#save as HMTL
writeLines(html_code_a5, here("report", "appendices", "appendix_5.html"))
